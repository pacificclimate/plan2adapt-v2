...
export const fetchCsvStats = (region, variable, season) => {
  const csvUrl = urljoin(
    window.env.REACT_APP_STATS_URL,
    `${regionId(region)}.csv`,
  );
  ...
  return axios
    ...
    .then((data) => {
      ...
      if (!meanEntry) {
        console.error(
          `No matching entry found for ${variable} in season ${season}`,
        );
        return null;
      }
    ...
    .catch((error) => {
      console.error(
        `Error fetching or parsing CSV data from ${csvUrl} for ${variable}, season ${season}:`,
        error,
      );
      return null;
    });
